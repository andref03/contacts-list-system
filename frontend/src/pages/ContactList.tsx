  ...
  return (
    <div className="w-full min-h-screen bg-gray-900 flex flex-col items-center px-4 py-10">
      ...
      <div className="w-full md:max-w-2xl mb-4 flex gap-2">
        <input value={query} onChange={(e) => { setQuery(e.target.value); setPage(1); }} placeholder="Buscar por nome ou email..." className="flex-1 p-2 rounded" />
      </div>
      ...
      <ul className="w-full md:max-w-2xl space-y-2">
        {contacts.map((c) => (
          <li key={c.id} className="bg-gray-800 p-3 rounded text-white flex justify-between items-center">
            <div>
              <strong>{c.name}</strong> — {c.email} ({c.phone || "sem telefone"})
            </div>
            <div className="space-x-2">
              <button onClick={() => navigate(`/edit/${c.id}`)} className="bg-yellow-500 text-white px-2 py-1 rounded hover:bg-yellow-400 transition">Editar</button>
              <button onClick={() => handleDelete(c.id)} className="text-bold bg-red-500 px-2 py-1 rounded hover:bg-red-600">Excluir</button>
            </div>
          </li>
        ))}
      </ul>
      ...
      <div className="flex gap-2 mt-4">
        <button disabled={page === 1} onClick={() => { setPage((p) => { const v = p - 1; load(query, v); return v; }); }} className="px-3 py-1 bg-gray-700 text-white rounded disabled:opacity-50">Anterior</button>
        {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
          <button key={p} onClick={() => { setPage(p); load(query, p); }} className={`px-3 py-1 rounded ${p === page ? "bg-yellow-500 text-white" : "bg-gray-700 text-white"}`}>{p}</button>
        ))}
        <button disabled={page === totalPages} onClick={() => { setPage((p) => { const v = p + 1; load(query, v); return v; }); }} className="px-3 py-1 bg-gray-700 text-white rounded disabled:opacity-50">Próximo</button>
      </div>
